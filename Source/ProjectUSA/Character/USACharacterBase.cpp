...
FVector AUSACharacterBase::GetUSACharacterDirection_Target()
{
	...
	//First of all, if it is a null pointer, it is judged as temporary targeting.
	...
	//In case of temporary targeting, restoration
...
void AUSACharacterBase::PossessedBy(AController* NewController)
{
	...
	//Enter console automatically at startup
...
void AUSACharacterBase::Move(const FInputActionValue& Value)
{
	...
	if (Controller != nullptr)
	{
		...
		//Variables to use to improve operation feel
...
{
	//Perform all server configurations
...
//TODO: Fix issue with weak falling due to gravity in the future
...
float AUSACharacterBase::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	//If the opponent is invincible, it does not apply if the opponent is knocked down.
	...
	//Because it is performed only on the server, players and AI controllers can be utilized.
	...
	//Damage is applied except when parrying.
	if (IsValid(ASC) == true
		&& ASC->HasMatchingGameplayTag(USA_CHARACTER_ACTION_PARRY) == false)
	{
		...
		//MulticastRPC to perform CameraShake when the assailant is hit by an opponent
...
{
	//Perform camera shake by offender
...
{
	//Perpetrator performs CameraShake
	if (AUSACharacterBase* CharacterCauser = Cast<AUSACharacterBase>(DamageCauser))
	{
		//StartCameraShake_HitSuccess() is a function created in virtual form.
		//CameraShake process is implemented through override in Player.
		CharacterCauser->StartCameraShake_HitSuccess(DamageEvent.DamageTypeClass);
	}
}
...
//It only works on servers that perform actual physics calculations, and the caller must use the TakeDamage() function
	...
	AActor* DamageCauser)
{
	...
	//Ignores knockback when dead
	if (ASC->HasMatchingGameplayTag(USA_CHARACTER_STATE_DEAD))
	{
		return;
	}

	//Determine death status by checking the character's current health
	...
	//Check if the current character is parrying
	...
	//Variables to determine direction of movement
	FVector NewDirection = FVector::ForwardVector;

	//Initializing variables to retrieve attack information
	...
	//Calculate direction of movement via DamageType
	...
	{
		//In case of a normal attack, determine the direction of movement using AttackDirection of DamageEvent.
		NewDirection = AttackDirection * -1.0f;
		NewDirection.Z = 0.0f;
		NewDirection.Normalize();
	}

	//Decide which knockback ability to perform
	if (bIsJustDead)
	{
	...
	//Transfer direction of movement and knockback ability to server and client
...
void AUSACharacterBase::ApplyDamageHitNiagaraEffect(AController* EventInstigator, AActor* DamageCauser, UNiagaraSystem* SystemTemplate, bool bIsOffset)
{
	...
	//Prevent team kill
	//Since it is performed only on the server, a controller can be used.
	AAIController* EventAIController = Cast<AAIController>(EventInstigator);
	APlayerController* EventPlayerController = Cast<APlayerController>(EventInstigator);
	...
	//Same team (the reason we check the same controllers is to implement attacks that match each other)
	if (MyAIController && EventAIController && (MyAIController != EventAIController))
	{
		return;
	}
...
void AUSACharacterBase::OnRep_CurrentOwnedItems(TArray<TSubclassOf<AUSAItemBase>> PrevItems)
{
	...
	{
		//Since it always adds backwards...
